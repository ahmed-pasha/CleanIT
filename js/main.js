...
const cityNames = ["Household Waste", "Plastic Collection", "Organic Waste", "Recyclable Materials"];
let index = 0;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  detectLocationButton.textContent = "Detect Location";
  detectLocationButton.type = "button";
  detectLocationButton.classList.add("detect-location-btn");
  
  const localityField = document.getElementById("locality");
  localityField.parentNode.insertBefore(detectLocationButton, localityField.nextSibling);
      ...
      if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(
              (position) => {
                  ...
                  // Use reverse geocoding to find locality
                  fetch(`https://nominatim.openstreetmap.org/reverse?lat=${latitude}&lon=${longitude}&format=json`)
                      .then((response) => response.json())
                      .then((data) => {
                          const locality = data.address?.suburb || data.address?.city || "Unknown Locality";
                          localityField.value = locality; // Autofill locality field
                          alert(`Location detected: ${locality}`);
                      })
                      .catch((error) => {
                          console.error("Error during reverse geocoding:", error);
                          alert("Could not determine locality. Please enter it manually.");
                      });
              },
              (error) => {
